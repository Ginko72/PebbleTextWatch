...
// Auto-generated by build from package.json messageKeys
extern uint32_t MESSAGE_KEY_TimeRenderOh;
extern uint32_t MESSAGE_KEY_DateSeparatorLine;
extern uint32_t MESSAGE_KEY_DateOutsideJustified;
extern uint32_t MESSAGE_KEY_DateFormatUS;
...
static void layout_config_init(LayoutConfig *cfg, GRect bounds, bool date_outside_just) {
...
void date_to_string(struct tm *t, char *line, size_t length) {
    ...
    const char *fmt = s_settings.date_format_us ? "%m.%d" : "%d.%m";
    strftime(line, length, fmt, t);
...
void day_of_week(struct tm *t, char *line, size_t length) {
    ...
    if (!s_settings.date_outside_just) line[2] = 0;
}
...
void display_time(struct tm *t) {
    ...
    time_to_3words(t->tm_hour, t->tm_min, textLine1, textLine2, textLine3, BUFFER_SIZE,
                   layout.screen_width < 200, s_settings.time_render_oh);
...
static void prv_apply_settings(void) {
    ...
    layout_config_init(&layout, layer_get_bounds(root_layer), s_settings.date_outside_just);

    layer_set_frame(text_layer_get_layer(line4.currentLayer),
        GRect(layout.weekday_x, layout.weekday_y, layout.weekday_w, layout.line_height));
    text_layer_set_text_alignment(line4.currentLayer,
        layout.weekday_right ? GTextAlignmentRight : GTextAlignmentLeft);

    layer_set_frame(text_layer_get_layer(line5.currentLayer),
        GRect(layout.date_x, layout.date_y, layout.date_w, layout.line_height));
    text_layer_set_text_alignment(line5.currentLayer,
        layout.date_right ? GTextAlignmentRight : GTextAlignmentLeft);

    layer_set_hidden(lineDrawLayer, !s_settings.date_separator_line);
...
static void prv_inbox_received(DictionaryIterator *iter, void *context) {
    ...
    t = dict_find(iter, MESSAGE_KEY_TimeRenderOh);
    if (t) s_settings.time_render_oh = t->value->int32 != 0;

    t = dict_find(iter, MESSAGE_KEY_DateSeparatorLine);
    if (t) s_settings.date_separator_line = t->value->int32 != 0;

    t = dict_find(iter, MESSAGE_KEY_DateOutsideJustified);
    if (t) s_settings.date_outside_just = t->value->int32 != 0;

    t = dict_find(iter, MESSAGE_KEY_DateFormatUS);
    if (t) s_settings.date_format_us = t->value->int32 != 0;
